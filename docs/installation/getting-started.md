...
??? info "Configuration (Environment variables)"
    ...
    | `HOST`            | `0.0.0.0`                                                                                                        | The bind address for the server.                                                                                        |
    ...
    | `WEB_PATH`        | (none)                                                                                                           | The folder containing the web client files to serve. By default these files are provided by the server binary.         |

    Each environment variable above maps to its corresponding key in the YAML config file. Values set in the
    YAML file take precedence over environment variables. For qBittorrent and OIDC configuration, see
    [Connecting qBittorrent](../4-connecting-qbittorrent.md) and [OIDC Authentication](../5-oidc-authentication.md).
