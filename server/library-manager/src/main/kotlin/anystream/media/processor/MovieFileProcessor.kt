...
class MovieFileProcessor(
    ...
    private suspend fun findMatchesForRootDir(
        mediaLink: MediaLink,
        import: Boolean,
    ): MediaLinkMatchResult {
        ...
        val subResults = childLinks.mapNotNull { childLink ->
            when (childLink.descriptor) {
                Descriptor.VIDEO -> {
                    findMatchesForFile(childLink, import)
                }

                Descriptor.AUDIO -> {
                    error("AUDIO links are not supported in movie libraries")
                }
    ...
    private suspend fun findMatchesForFile(
        ...
        val (movieName, year) = when (val result = fileNameParser.parseFileName(movieFile)) {
            is ParsedFileNameResult.MovieFile -> {
                result
            }
    ...
    private suspend fun getOrImportMetadata(metadataMatch: MetadataMatch.MovieMatch): MetadataMatch.MovieMatch? {
