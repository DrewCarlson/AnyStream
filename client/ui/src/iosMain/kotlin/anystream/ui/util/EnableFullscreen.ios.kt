/**
 * AnyStream
 * Copyright (C) 2025 AnyStream Maintainers
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program.  If not, see <http://www.gnu.org/licenses/>.
 */
package anystream.ui.util

import androidx.compose.runtime.Composable
import androidx.compose.runtime.DisposableEffect
import androidx.compose.runtime.staticCompositionLocalOf

@Composable
actual fun EnableFullscreen(isUserRequested: Boolean) {
    val systemBarController = LocalSystemBarController.current
    DisposableEffect("enable-fullscreen") {
        systemBarController.setSystemBars(hidden = true)
        onDispose {
            systemBarController.setSystemBars(hidden = false)
        }
    }
}

val LocalSystemBarController = staticCompositionLocalOf<SystemBarController> {
    error("SystemBarController not provided")
}

interface SystemBarController {
    fun setSystemBars(hidden: Boolean)
}
